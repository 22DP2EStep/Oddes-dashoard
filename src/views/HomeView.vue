        ...
        <div class="pipeline-section">
          ...
          <div class="pipeline-container">
            ...
            >
              ...
              <div
                class="stage-bar"
                :style="{ width: getStageWidth(stage.value) + '%' }"
              ></div>
            </div>
          </div>
        </div>
...
const pipelineStages = ref([
  {
    name: "# of Attempted Contact (#1, #2, #3)",
    value: 1250,
    status: "Active",
  },
  { name: "# of Has Solar, Not Interested", value: 420, status: "Inactive" },
  {
    name: "# of Has Solar, Has Batteries, Not Interested",
    value: 180,
    status: "Pending",
  },
  ...
  try {
    ...
    if (tableStats.totalRecords > 0) {
      ...
      if (tableStats.averageValue) {
        ...
        metrics.value.avgCostPerCall =
          Math.round(tableStats.averageValue * 0.1 * 100) / 100;
      }

      // Calculate rates
      if (tableStats.totalRecords > 0) {
        const pickupRate =
          (tableStats.recentRecords / tableStats.totalRecords) * 100;
    ...
    error.value =
      "Failed to load data from Supabase. Please check your table configuration.";
  } finally {
    loading.value = false;
  }
};
